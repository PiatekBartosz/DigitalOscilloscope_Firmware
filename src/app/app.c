...
int app_set_sample_size(uint16_t count)
{
    ...
    do
    {
        ...
        ret = parallel_link_set_sample_size(count);
        if (ret == 0)
        {
            s_sample_size = count;
...
int app_set_pretrigger(uint16_t count)
{
    ...
    do
    {
        ...
        ret = parallel_link_set_pretrigger(count);
        if (ret == 0)
        {
...
int app_acquire(uint16_t *ch1, uint16_t *ch2)
{
    ...
    do
    {
        ...
        ret = parallel_link_acquire(ch1, ch2, s_sample_size, ACQUIRE_TIMEOUT_MS);
    } while (false);
