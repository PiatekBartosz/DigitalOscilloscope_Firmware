 ...
 * ============================================================ */
#define DAC61408_DEVICEID_MAGIC 0x0920
...
#define DAC61408_RANGE_ALL_PM5V 0x9999
							  ...
							  uint16_t data)
{
	...
	uint8_t tx_buf_raw[3] = {0};
	uint8_t rx_buf_raw[3] = {0};
	/* Bit7 = 0 for WRITE */
	tx_buf_raw[0] = addr & 0x7F;
	sys_put_be16(data, &tx_buf_raw[1]);

	struct spi_buf tx_buf =
		{
			.buf = tx_buf_raw,
			.len = sizeof(tx_buf_raw),
		};
	...
	LOG_DBG("WR RAW send: 0x%02X 0x%02X 0x%02X", tx_buf_raw[0], tx_buf_raw[1], tx_buf_raw[2]);
	LOG_DBG("WR RAW recv: 0x%02X 0x%02X 0x%02X", rx_buf_raw[0], rx_buf_raw[1], rx_buf_raw[2]);
							 ...
							 uint16_t *data)
{
	...
	LOG_DBG("RD0 RAW send: 0x%02X 0x%02X 0x%02X", tx_buf[0], tx_buf[1], tx_buf[2]);
	LOG_DBG("RD0 RAW recv: 0x%02X 0x%02X 0x%02X", rx_buf[0], rx_buf[1], rx_buf[2]);
	...
	*data = sys_get_be16(&rx_buf[1]);
	...
	LOG_DBG("RD1 RAW send: 0x%02X 0x%02X 0x%02X", tx_buf[0], tx_buf[1], tx_buf[2]);
	LOG_DBG("RD1 RAW recv: 0x%02X 0x%02X 0x%02X", rx_buf[0], rx_buf[1], rx_buf[2]);
...
static int dac61408_init(const struct device *dev)
{
	...
	do
	{
		...
		ret = dac61408_reg_update_bits(dev,
									   ...
									   DAC61408_SPI_SDO_EN_MASK | DAC61408_SPI_DEV_PWDWN_MASK,
									   DAC61408_SPI_SDO_EN_MASK);
		if (ret != 0)
		{
			LOG_ERR("Failed to configure SPICONFIG");
			break;
		}
		...
		if (device_id != DAC61408_DEVICEID_MAGIC)
		{
			LOG_ERR("Device ID: 0x%04X, did not match expected: 0x%04X", device_id, DAC61408_DEVICEID_MAGIC);
			break;
		}
		else
		{
		...
		if (ret != 0)
		{
			LOG_ERR("Failed to write genconfig");
			break;
		}

		// TODO: this probably should be fetched from device tree
		/* Setup -5V +5V range */
		ret = dac61408_reg_write(dev,
								 DAC61408_REG_DACRANGE0,
								 DAC61408_RANGE_ALL_PM5V);
		if (ret != 0)
		{
			LOG_ERR("Failed to enable -5V +5V range");
			break;
		}

		ret = dac61408_reg_write(dev,
								 DAC61408_REG_DACRANGE1,
								 DAC61408_RANGE_ALL_PM5V);
		if (ret != 0)
		{
			LOG_ERR("Failed to enable -5V +5V range");
		...
		// TODO: fix this loop, add constant for n of channels
		/* Deafult to 0V */
		for (int channel = 0; channel < 7; ++channel)
		{
			ret |= dac61408_write_value(dev, channel, 0x800);
		}
		if (ret != 0)
		{
			LOG_ERR("Setting startup dac values to 0V failed");
			break;
		}
		...
		if (ret != 0)
		{
			LOG_ERR("Failed to power up dacs");
			break;
		}
